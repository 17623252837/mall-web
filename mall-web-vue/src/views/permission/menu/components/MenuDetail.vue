...
<script>
  ...
  export default {
    ...
    created() {
      if (this.isEdit) {
        getMenu(this.$route.query.id,this.$store.state.user.token).then(response => {
    ...
    methods: {
      getSelectMenuList() {
        fetchList(0, {pageSize: 100, pageNum: 1},this.$store.state.user.token).then(response => {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$confirm('是否提交数据', '提示', {
              ...
              if (this.isEdit) {
                updateMenu(this.$route.query.id, this.menu,this.$store.state.user.token).then(response => {
                  this.$message({
                    message: '修改成功',
                    type: 'success',
                    duration: 1000
                ...
                createMenu(this.menu,this.$store.state.user.token).then(response => {
