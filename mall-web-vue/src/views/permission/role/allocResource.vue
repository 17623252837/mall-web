...
<script>
  ...
  export default {
    ...
    methods: {
      getAllResourceList() {
        fetchAllResourceList(this.$store.state.user.token).then(response => {
      ...
      getResourceByRole(roleId){
        listResourceByRole(roleId,this.$store.state.user.token).then(response=>{
      ...
      handleSave() {
        this.$confirm('是否分配资源？', '提示', {
          ...
          allocResource(params,this.$store.state.user.token).then(response => {
            this.$message({
              message: '分配成功',
              type: 'success',
              duration: 1000
