...
<script>
  ...
  export default {
    ...
    methods: {
      treeList() {
        fetchTreeList(this.$store.state.user.token).then(response => {
      ...
      getRoleMenu(roleId){
        listMenuByRole(roleId,this.$store.state.user.token).then(response=>{
      ...
      handleSave() {
        ...
        this.$confirm('是否分配菜单？', '提示', {
          ...
          allocMenu(params,this.$store.state.user.token).then(response => {
            this.$message({
              message: '分配成功',
              type: 'success',
              duration: 1000
